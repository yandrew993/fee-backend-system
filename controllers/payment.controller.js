...
export const getPayments = async (req, res) => {
  try {
    ...
    // Build where clause based on userId if provided
...
export const totalPayments = async (req, res) => {
  try {
    const total = await prisma.feePayment.aggregate({
      _sum: {
        amount: true
      }
    });
...
export const getPaymentStats = async (req, res) => {
  try {
    ...
    const totalResult = await prisma.feePayment.aggregate({
      _sum: {
        amount: true
      }
    });
    ...
    const previousMonthTotal = await prisma.feePayment.aggregate({
      _sum: {
        amount: true
      },
      where: {
    ...
    const currentMonthTotal = await prisma.feePayment.aggregate({
      _sum: {
        amount: true
      },
      where: {
    ...
    const weeklyTotal = await prisma.feePayment.aggregate({
      _sum: {
        amount: true
      },
      where: {
    ...
    const previousWeekTotal = await prisma.feePayment.aggregate({
      _sum: {
        amount: true
      },
      where: {
...
export const getMonthlyPaymentStats = async (req, res) => {
  try {
    ...
    // Build where clause - filter by studentId if provided
    const whereClause = {
      createdAt: {
    ...
    // If studentId is provided, add student filter
    if (studentId) {
      whereClause.studentId = studentId;
    }
    
    // Get all payments for the current year (filtered by student if provided)
